...
if (process.argv.length !== 4) {
  console.error(
    "Usage: bin/copySingle <source resource url> <destination API url>"
  )
...
class ResourceCopier {
  constructor(sourceResourceUrl, destApiUrl) {
    ...
    this.sourceApiUrl = this.sourceResourceUrl.slice(
      0,
      this.sourceResourceUrl.indexOf("/resource")
    )
  ...
  postResource(resource) {
    const newResource = JSON.parse(
      resource.replace(new RegExp(this.sourceApiUrl, "g"), this.destApiUrl)
    )
    return superagent
