...
if (process.argv.length !== 4 && process.argv.length !== 5) {
  console.error(
    "Usage: bin/copy <source API url> <destination API url> [source querystring (optional)]"
  )
...
class ResourcesCopier {
  ...
  async copy() {
    let nextUrl = this.querystring
      ? `${this.sourceApiUrl}/resource?${this.querystring}`
      : `${this.sourceApiUrl}/resource`
    while (nextUrl) {
  ...
  postResource(resource) {
    const newResource = JSON.parse(
      JSON.stringify(resource).replace(
        new RegExp(this.sourceApiUrl, "g"),
        this.destApiUrl
      )
    )
    ...
    return superagent
      ...
      .then((response) => {
        if (!response.ok) {
          throw new Error(
            `Post failed with ${response.status} for ${newResource.uri}`
          )
        }
