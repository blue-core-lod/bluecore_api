...
const { SharedIniFileCredentials, CognitoIdentityServiceProvider, Config } = awsSdkPkg
...
export default class AuthenticateClient {
  constructor (username, password) {
    ...
    this.appClientId = process.env.COGNITO_CLIENT_ID || '2u6s7pqkc1grq1qs464fsi82at'
    this.cognitoTokenFile = '.cognitoToken'
    this.awsRegion = process.env.AWS_REGION || 'us-west-2'
    this.cognitoIss = process.env.AWS_COGNITO_ENDPOINT || `https://cognito-idp.${this.awsRegion}.amazonaws.com/${this.userPoolId}`
  ...
  userSubFromCognitoPool (cognitoUserName) {
    ...
    return cognito.adminGetUser(desiredUserParams).promise().then((data, err) => {
    ...
    return new Promise((resolve, reject) => {
      this.cognitoUser().authenticateUser(this.authenticationDetails(), {
        onSuccess: (result) => {
          ...
          if (jwt) resolve(jwt)
          else reject(new Error(`ERROR: retrieved null cognito access token for ${this.username}`))
        },
        onFailure: (err) => {
          reject(err)
        }
