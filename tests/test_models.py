...
triple_embeddings = json.loads(
    pathlib.Path("tests/blue-core-triple-embeddings.json").read_text()
)
...
def test_triple_vector_index(pg_session):
    with pg_session() as session:
        triple_vector_index = (
            session.query(TripleVectorIndex).where(TripleVectorIndex.id == 1).first()
        )
        work_version = session.query(Version).where(Version.id == 1).first()
        assert triple_vector_index.version == work_version
        assert len(triple_vector_index.vector) == VECTOR_SIZE
        assert triple_vector_index.created_at
        assert triple_vector_index.updated_at
        triple_vector_index2 = (
            session.query(TripleVectorIndex).where(TripleVectorIndex.id == 2).first()
        )
        instance_version = session.query(Version).where(Version.id == 2).first()
        assert triple_vector_index2.version == instance_version
        assert len(triple_vector_index2.vector) == VECTOR_SIZE
        assert triple_vector_index.vector[0] != triple_vector_index2.vector[0]
